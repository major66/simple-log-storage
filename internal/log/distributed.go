...
// if need strong consistency then go through Raft but reads will be less efficient.
...
func (l *logStore) StoreLogs(records []*raft.Log) error {
	for _, record := range records {
		if _, err := l.Append(&api.Record{Value: record.Data, Term: record.Term, Type: uint32(record.Type)}); err != nil {
			return err
		}
	}
...
// Accept is the mirror of Dial. It accepts the incoming connection and read the byte that identifies the connection and then create a server-side TLS connection.
